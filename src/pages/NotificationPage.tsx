...
interface Notification {
  id: string;
  type: "security" | "database" | "revenue" | "flag" | string;
  title: string;
  body: string;
  createdAt: string;
  read: boolean;
...
const NotificationIcon = ({ type }: { type: string }) => {
  const styles: any = {
    security: { bg: "bg-rose-50", color: "text-rose-500", icon: "🔒" },
    database: { bg: "bg-emerald-50", color: "text-emerald-500", icon: "🗄️" },
    revenue: { bg: "bg-purple-50", color: "text-purple-500", icon: "$" },
    flag: { bg: "bg-orange-50", color: "text-orange-500", icon: "🚩" },
  };

  const current = styles[type] || styles.security;

  return (
    <div
      className={`w-12 h-12 rounded-2xl ${current.bg} ${current.color} flex items-center justify-center text-xl`}
    >
      {current.icon}
    </div>
  );
      ...
      <div className="space-y-4">
        {notifications.length > 0 ? (
          notifications.map((notif) => (
            <div
              key={notif.id}
              className="p-4 border rounded-xl flex gap-4 items-center"
            >
