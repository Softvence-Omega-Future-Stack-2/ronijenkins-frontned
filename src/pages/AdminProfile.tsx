...
const ProfileHeaderSkeleton = () => (
  <div className="bg-white rounded-2xl md:rounded-4xl border border-borderColor shadow-sm p-5 sm:p-6 animate-pulse">
    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
      <div className="flex items-center gap-4">
        <div className="w-16 h-24 sm:w-[119px] sm:h-[119px] rounded-2xl bg-gray-200" />
...
const ProfileHeader = ({ username, email, contactNo, avatar, role }: AdminData) => {
  const [, setEditHovered] = useState(false);
  const navigate = useNavigate();
  const roleLabel = role?.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) ?? "Admin";
    ...
    <div className="bg-white rounded-2xl md:rounded-4xl border border-borderColor shadow-sm p-5 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-titleColor leading-6 md:leading-10 tracking-tight">
              {username}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 mt-4">
              <div className="flex items-center gap-2">
                <div className="p-2 rounded-lg bg-[#9266901A] text-buttonColor"><MailIcon /></div>
                <div>
                  <p className="text-xs text-subTitleColor uppercase tracking-wider font-black">Email</p>
                  <p className="text-sm text-titleColor leading-5 font-medium">{email}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="p-2 rounded-lg bg-[#00C9501A] text-green-500"><PhoneIcon /></div>
                <div>
                  <p className="text-xs text-subTitleColor uppercase tracking-wider font-black">Phone</p>
                  <p className="text-sm text-titleColor leading-5 font-medium">{contactNo || "—"}</p>
                </div>
              </div>
            </div>
          </div>
...
export default function AdminProfile() {
  // After transformResponse, `admin` is already the `me` object directly
  const { data: admin, isLoading, isError } = useGetAdminProfileQuery();
    console.log("📊 Query state:", { admin, isLoading, isError,  });
  ...
  return (
    <div className="min-h-screen p-4 md:p-6">
      <div className="space-y-4">
        ...
        {isError && (
          <div className="bg-red-50 border border-red-200 text-red-600 rounded-2xl p-5 text-sm font-medium">
            Failed to load profile. Please refresh the page or try again.
          </div>
        )}

      {!isLoading && !isError && admin && (
  <ProfileHeader {...admin} />   // ✅ no (admin as AdminData) needed
)}
