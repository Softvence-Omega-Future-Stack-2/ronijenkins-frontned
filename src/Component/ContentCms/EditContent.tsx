...
// ✅ type থেকে dropdown label এ map
...
// ✅ category uppercase থেকে readable label এ map
...
const EditContentForm: React.FC = () => {
  ...
  // ✅ Store থেকে existing item load করা
  ...
  const [typeSelected, setTypeSelected] = useState(typeToLabel(item?.type || 'article'));
  const typeOptions = ['Article', 'Video', 'Meditation'];

  const [categoryOpen, setCategoryOpen] = useState(false);
  const [categorySelected, setCategorySelected] = useState(
    item?.category ? categoryToLabel(item.category) : 'Symptom Relief'
  );
  ...
  const handleUpdate = () => {
    ...
    navigate(-1); // ✅ back to ContentCMS, updated data দেখাবে
  };
