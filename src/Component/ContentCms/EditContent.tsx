...
import { useGetContentByIdQuery, useUpdateContentMutation } from '../../redux/features/admin/content/contentApi';
...
const uiTypeToServer: Record<ContentType, string> = {
  Article: 'ARTICLE',
  Video: 'VIDEO',
  Audio: 'AUDIO',
};
...
function Dropdown<T extends string>({
  ...
  return (
    <div className="space-y-2 relative">
      ...
      <div className="w-full bg-[#FAF7F5] border border-borderColor rounded-2xl p-4 flex justify-between items-center cursor-pointer" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <div className="absolute z-10 w-full mt-1 bg-white border border-borderColor rounded-xl shadow-md">
          {options.map((o) => (
            <div key={o} onClick={() => { onChange(o); setOpen(false); }} className="p-3 hover:bg-[#F2F1EE] cursor-pointer text-sm">
...
const EditContentForm: React.FC = () => {
  ...
  if (item && !initialized.current) {
    ...
    setTypeSelected(serverTypeToUI(item.type || 'ARTICLE'));
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast.update(toastId, {
        render: err?.data?.errors?.[0]?.message || 'Failed to update content.',
        type: 'error',
        isLoading: false,
        autoClose: 3000,
        position: 'top-right',
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-[40px] shadow-sm border border-gray-100 p-8 md:p-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
          <div>
            ...
            <p className="text-subTitleColor text-sm font-medium leading-5 mt-0.5">Update and republish to Global Library</p>
          </div>
        ...
        <div className="mt-10">
          ...
          {typeSelected === 'Audio' && (
            <div className="space-y-3">
              ...
              <MeditationEditor
                onChange={(data) => setMeditationData(data)}
              />
            </div>
          )}
        </div>
